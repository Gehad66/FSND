...
class Venue(db.Model):
    ...
    def __repr__(self):
      return f'<Venue ID {self.id}, name {self.name}, city {self.city}, state {self.state},\
              ...
              seeking_talent {self.seeking_talent}, seeking_description {self.seeking_description},\
              genres {self.genres}>'
...
# genre_items = db.Table('genre_items',
...
#   id = db.Column(db.Integer, primary_key=True)
#   name = db.Column(db.String, nullable = False)
#   venues = db.relationship('Venue', secondary=genre_items,
...
def venues():
  ...
  for venue in venues:
    ...
    num_upcoming_shows = db.session.query(Venue).join(Show).filter(Venue.id == venue.id, Show.start_time > current_time).count()
...
def search_venues():
  ...
  response = {}
  try:
    venues = Venue.query.with_entities(Venue.id, Venue.name).filter(Venue.name.ilike('%' + request.form.get('search_term', '') +
                                '%')).all()
...
def create_show_submission():
  ...
  # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
  if error:
    flash('An error occurred. Show ' + request.form['name'] +
              ' could not be listed.')
    return render_template('forms/new_show.html', form=form)
  # on successful db insert, flash success
